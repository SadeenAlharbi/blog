...
@section('title', 'نسيت كلمة المرور — منصة المعرفة السعودية')

@section('content')
    <div class="text-center mb-8">
        <a href="{{ url('/') }}" class="inline-flex mb-4"><x-logo :size="56" /></a>
        <h1 class="text-2xl font-bold text-ink-900">نسيت كلمة المرور؟</h1>
        <p class="text-sm text-ink-500 mt-1.5">أدخل بريدك الإلكتروني وسنرسل لك رابطاً لإعادة تعيين كلمة المرور.</p>
    </div>

    <form method="POST" action="{{ route('password.email') }}" class="space-y-5 bg-white border border-ink-100 rounded-2xl p-6 shadow-sm">
        @csrf
